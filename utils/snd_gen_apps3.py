...
def gen_times(start_time, stop_time):
    ...
    if stop_time is None:
        duration = randrange(FLOW_DURATION[0], FLOW_DURATION[1] + 1) * 60
        stop = start + duration
    else:
        stop = stop_time
...
def random_more_value(id, total_bps, start_time, stop_time):
    if total_bps < FLOW_RATES[1]:
        ...
        rest = 0
        bps = 0
        while rest < FLOW_RATES[0]:
            bps = randrange(FLOW_RATES[0], FLOW_RATES[1] + 1)
            rest = total_bps - bps
...
def random_less_value(id, lapps, total, value, start_time, stop_time):
    while total > value:
        ...
        for index in range(len(lapps)):
            app = lapps[index]
            new_total = total - app
            if new_total > value or (value - new_total) >= FLOW_RATES[0]:
                fix_app_end(app)
                lapps.pop(index)
                total = new_total
                removed = True
    ...
    if total < value:
        while lapps and (value - total) < FLOW_RATES[0]:
            oldest_app = lapps.pop(0)
            fix_app_end(oldest_app)
            total -= oldest_app
...
def main():
    ...
    FLOW_RATES = [int(quart[-1]), int(perc[-1])]
    ...
    with open("applications-3.toml", "wb") as output:
