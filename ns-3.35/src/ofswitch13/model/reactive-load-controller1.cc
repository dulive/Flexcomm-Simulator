...
ReactiveLoadController1::CalculatePath (Flow flow)
{
  ...
  if (flow.eth_type == ETH_TYPE_IP)
    {
      path = Topology::DijkstraShortestPath<double> (flow.src_ip, flow.dst_ip, weight_calc);
    }
  else
    {
      path = Topology::DijkstraShortestPath<double> (flow.src_mac, flow.dst_mac, weight_calc);
    }
  return path;
}
